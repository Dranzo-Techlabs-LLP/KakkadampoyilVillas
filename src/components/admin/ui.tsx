...
export function Modal({ open, onClose, title, children, wide }: {
  ...
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto overscroll-contain bg-slate-900/40 p-2 backdrop-blur-sm sm:p-4 md:p-8">
      <div className={`relative my-2 w-full ${wide ? "max-w-3xl" : "max-w-lg"} rounded-2xl bg-white shadow-xl sm:my-4`}>
        <div className="sticky top-0 z-10 flex items-center justify-between rounded-t-2xl border-b border-slate-200 bg-white px-4 py-3 sm:px-6 sm:py-4">
          <h3 className="text-base font-semibold text-slate-900 sm:text-lg">{title}</h3>
          <button onClick={onClose} aria-label="Close" className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-600">✕</button>
        </div>
